...
import { LRU, traceKey } from '../../tracing/cache.ts';
import { TraceStore } from '../../tracing/store.ts';
import type { TraceLimits, TraceResult, TraceStep } from '../../tracing/schema.ts';

/**
 * Python tracing, from the page's side.
 *
 * Where it runs: in the browser, in a Web Worker, on Pyodide — the same
 * runtime Officina already serves from its own origin. Nothing is sent to a
 ...
 * site does not enable (public/_headers in the portfolio says why). So the
