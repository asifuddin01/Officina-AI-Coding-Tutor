...
import { TRACER, program, traceNatively } from './native.ts';
...
const sameVersion = (() => {
  try {
    return execFileSync('python3', ['-c', 'import sys; print(sys.version_info[:2] == (3, 14))'], { encoding: 'utf8' }).trim() === 'True';
  } catch {
    return false;
  }
})();
...
test('a ~5,000-step program traces within the 500 ms budget (§27.8)', () => {
  ...
  assert.ok(median < 500, `median ${median.toFixed(0)} ms`);
});
